...
let configpath = path.resolve(__dirname, "ali2wxapp.txt");
let dirPath="";
if(arg[2]){
    if(arg[2]==="--start"){
        ...
        if(arg[3]){
            ...
            console.log("请输入工作目录 如：ali2wxapp --path 'c:\\ab'");
            return ;
        }
    }
    else{
        console.log("你可以输入ali2wxapp --getConfig 获取配置文件路径 然后修改配置文件");
        console.log("你可以输入ali2wxapp --path <path路径> 以输入的path为工作路径开始转换");
        console.log("你可以输入ali2wxapp --start开始转换(以配置中的路径为工作目录)");
        return;
    }
}else{
    console.log("你可以输入ali2wxapp --getConfig 获取配置文件路径 然后修改配置文件");
